            ...
            <div class="row justify-content-between">
                <div class="col-lg-8 p-0 post">
                    <div class="bg-white px-4">
                        ...
                        <span class="post-date d-block mb-3"> {{ date('d-m-Y', strtotime($post->created_at)) }}</span>
                        <div class=" my-2">
                            نهاية التقديم
                            <span class="date red">{{ date('d-m-Y', strtotime($post->deadline)) }}</span>
                        </div>
                        <div class="  mt-4 mb-2">
                            للتواصل :
                        </div>
                        @if($post->recruiter_email)
                        <div class=" my-2">
                            البريد الالكتروني :
                            <span>{{ $post->recruiter_email }}</span>
                        </div>
                        @endif
                        @if($post->recruiter_phone)
                        <div class=" my-2">
                             رقم الجوال :
                            <span >{{ $post->recruiter_phone }}</span>
                        </div>
                        @endif


                        <div class="post-content mt-4 mb-2 pb-5">
                            {!! str_replace(PHP_EOL,'<br>',$post->desc) !!}
                        </div>
                        @if($post->image)
                            <div class="post-content mt-2 mb-2  pb-5">
                                <img src="{{url('user_files/'.$post->image)}}" class="img-fluid" alt="">
                            </div>
                        @endif
                    </div>
